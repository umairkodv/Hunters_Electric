...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin-layout','data' => ['title' => 'Edit Department']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Edit Department']); ?>
    <div class="max-w-lg bg-white border border-gray-200 rounded-2xl shadow-2xs p-6">
        <form action="<?php echo e(route('admin.departments.update', $department)); ?>" method="POST" class="flex flex-col gap-4">
            <?php echo csrf_field(); ?>
            ...
            <div>
                ...
                <p class="text-[10px] text-gray-400 font-semibold mt-1">Current slug: <span class="font-mono"><?php echo e($department->slug); ?></span>. Changing the name regenerates it, which changes this department's URL.</p>
            </div>

            <div>
                <label class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1">Sort Order</label>
                <input type="number" name="sort_order" value="<?php echo e(old('sort_order', $department->sort_order)); ?>"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-accent focus:border-accent" />
            </div>
...
<?php endif; ?>
<?php /**PATH C:\Laravel\Hunters_Electric\resources\views/admin/departments/edit.blade.php ENDPATH**/ ?>